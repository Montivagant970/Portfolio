...
def main(rank, world_size, config, resume, preload, pretrained_path):
    ...
    model = Wav2Vec2ForCTC.from_pretrained(
        ...
        # gradient_checkpointing=False #JAY turned off
    )
    ...
    if rank == 0:
        print("\n\nNumber of training utterances: ", len(train_ds))
...
if __name__ == '__main__':
    ...
    args.add_argument('-pp', '--pretrained_path', default=None, type=str,
                      help='Path to pretrained Model Directory with .json files')            
    ...
    mp.spawn(
        main,
        args = (n_gpus, config, args.resume, args.preload, pretrained_path),
        nprocs = n_gpus,
        join = True
    )
