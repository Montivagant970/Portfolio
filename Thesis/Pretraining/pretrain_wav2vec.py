...
class CustomDataset(Dataset):
    ...
    def load_ds(self, all_files):
        ...
        if isinstance(all_files, str): #JAY
            all_files = [all_files] #JAY
    ...
    def __getitem__(self, idx):
        ...
        #---
        # batch["input_values"] = np.mean(batch["input_values"], axis=1) #JAY Changes Stereo to Mono when not preprocessed
        #---
...
def main():
    ...
    # args = Args() #JAY Colab implimentation
    args = parse_args() 

    # Initialize the accelerator. We will let the accelerator handle device placement for us in this example.
    accelerator = Accelerator() #JAY Colab implimentation
    ...
    config.do_stable_layer_norm = True #JAY CHANGE (Wav2Vec2 - Base doesn't meet pretraining requirements for some unknown reason)
    config.feat_extract_norm = "layer" #JAY CHANGE
    ...
    if accelerator.is_main_process:
        print("\nNumber of training data: ", len(train_dataset))
