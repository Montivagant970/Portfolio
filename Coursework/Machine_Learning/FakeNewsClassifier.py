class FakeNewsClassifier:
  ...
  def __init__(self, data, stopwords, skipbucket, datasize = 1, test = False):
    ...
    ## counting token frequency and token totals for each category ##
    for category in self.categories:
      (self.prob[category], self.totals[category]) = self.train(buckets, category, skipbucket)
  ...
  ## methods ##
  def train(self, buckets, category, skipbucket):
